functionWrapper = @(X) (easomFunction(X(1), X(2))); 
...
% Plot results
graphWrapper = @(X1, X2) (easomFunction(X1, X2));
plotService = PlotService();
%figure();
%plotService.createGif(results, graphWrapper, [2 4], [2 4], 'x', 'y', 0.005, 50, 'figure.gif', 0.2, 'gif');
figure();
plotService.figureByIteration(1, results, graphWrapper, [-10 10], [-10 10], 'x', 'y', 0.005, 100);
figure();
if rem(results.iterations, 2) == 1
plotService.figureByIteration((results.iterations+1)/2, results, graphWrapper, [2 4], [2 4], 'x', 'y', 0.005, 100);
else
plotService.figureByIteration(results.iterations/2, results, graphWrapper, [2 4], [2 4], 'x', 'y', 0.005, 100);
end
figure();
plotService.figureByIteration(results.iterations, results, graphWrapper, [2 4], [2 4], 'x', 'y', 0.005, 100);
figure();
plotService.figure3D(graphWrapper, [-20 20], [-20 20], [-1 0.4], 'x', 'y', 'z', 'Easom', 200)
