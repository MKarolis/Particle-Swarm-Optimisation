...
for it=1:10
    ...
    if minResult > bestresultArray(it) && bestresultArray(it) ~= 0 % ~= 0 ?
...
plotService.createGif(results, graphWrapper, config.bounds, config.bounds, 'x', 'y', 4, 50, 'figure.gif', 2, 'gif');
plotService.figure3D(graphWrapper, config.bounds, config.bounds, config.bounds, 'x', 'y', 'z', 'title', 200)
